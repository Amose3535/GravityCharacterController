...
func _handle_jump() -> void:
	...
	if is_currently_on_floor:
		# Salto Immediato (tasto premuto mentre a terra) O Salto in Coda (buffer attivo)
		if jump_requested or jump_buffered:
			should_jump = true
	...
	if should_jump:
		...
		# Questo risolve il doppio salto, garantendo che l'input non venga riutilizzato
