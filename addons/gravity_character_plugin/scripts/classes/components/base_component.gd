...
## It's reccomended to have a unique identifier for your component so that a state machine (like the one provided) can reference that class script through the name directly.[br][br]
## [b][color=red]NOTE[/color]:[/b] [u]if you plan on using a ComponentStateMachine and have permanent components, please leave this field EMPTY.[/u]
...
## PROPOSAL: Add new "region_active" parameter to toggle off a component based on its region. 
##           This would be toggled using its name so again permanent components aren't affected.
